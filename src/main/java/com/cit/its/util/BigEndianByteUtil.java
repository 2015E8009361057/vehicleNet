...
public class BigEndianByteUtil extends ByteUtil{
	/**
	 * 将short转换为byte
	 * @param b 要转换进的byte数组
	 * @param s 需转换的short
	 ...
	 */
	public void putShort(byte[] b, short s, int pos) {
		b[pos] = (byte) ((s >> 8) & 0xFF);
		b[pos + 1] = (byte) (s & 0xFF);
	}
	...
	public short getShort(byte[] b, int pos) {
		return (short) (((b[pos + 0] << 8) | b[pos + 1] & 0xff));
	}
	 ...
	 * @return		pos应该后移的长度
	 */
	public  int putStringToByteArray(String str, byte[] bytes, int pos) {
	 ...
	 * @throws Exception
	 */
	public  String getStringFromByteArray(byte[] bytes, int pos, int length) throws Exception{
	 ...
	 */
	public  void putDateToByteArray(Date dateTime, byte[] bytes, int pos) throws ParseException {
	 ...
	 * @return 转换后的String日期
	 */
	public  String getStringDateFromByteArray(byte[] bytes, int pos) {
	 ...
	 * @param pos 放入数组的起始位置
	 */
	public  void putInt(byte[] b, int x, int pos) {
		b[pos + 0] = (byte) (x >> 24);
		b[pos + 1] = (byte) (x >> 16);
		b[pos + 2] = (byte) (x >> 8);
		b[pos + 3] = (byte) (x >> 0);
	...
	public  int getInt(byte[] b, int pos) {
		return (int) ((((b[pos + 0] & 0xff) << 24) | ((b[pos + 1] & 0xff) << 16) | ((b[pos + 2] & 0xff) << 8) | ((b[pos + 3] & 0xff) << 0)));
	}
     ...
     * @return byte[] 
     * */  
    public  byte[] intToByteArray(int s) {  
        byte[] targets = new byte[2];  
        for (int i = 0; i < 4; i++) {  
            int offset = (targets.length - 1 - i) * 8;  
            targets[i] = (byte) ((s >>> offset) & 0xff);  
    ...
    public  byte[] int2binaryBytes(int data) throws UnsupportedEncodingException {
    ...
    public  byte[] byte2binaryBytesArray(byte data) throws UnsupportedEncodingException {
